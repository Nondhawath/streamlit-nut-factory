...
def get_connection()
    try
        ...
        st.error(fเกิดข้อผิดพลาดในการเชื่อมต่อฐานข้อมูล {e})
        return None
...
def send_telegram_message(message)
    ...
    url = fhttpsapi.telegram.orgbot{token}sendMessagechat_id={chat_id}&text={message}
    try
        response = requests.get(url)
        if response.status_code != 200
            st.error(fTelegram แจ้งเตือนไม่สำเร็จ {response.status_code})
    except Exception as e
        st.error(fTelegram แจ้งเตือนไม่สำเร็จ {e})
...
def insert_job(data)
    ...
    with get_connection() as conn
        ...
        sql = fINSERT INTO job_tracking ({keys}) VALUES ({values})
...
def get_jobs_by_status_list(status_list)
    with get_connection() as conn
        ...
        sql = fSELECT  FROM job_tracking WHERE status IN ({qmarks}) ORDER BY created_at DESC
...
def transfer_mode(dept_from)
    ...
    if st.button(บันทึก Transfer)
        ...
        st.success(fบันทึก {dept_from} Transfer เรียบร้อยแล้ว)
...
def upload_wip_from_excel()
    ...
    if uploaded_file is not None
        ...
        if missing_columns
            st.error(fคอลัมน์ต่อไปนี้ขาดในไฟล์ Excel {', '.join(missing_columns)})
            return

        for col in optional_columns
            if col in df.columns and df[col].isnull().any()
                st.warning(fคอลัมน์ '{col}' มีข้อมูลที่เป็นค่าว่าง แต่ยังคงสามารถดำเนินการได้)
        ...
        def delete_existing_woc(woc_number)
            with get_connection() as conn
                ...
                if count  0
                    ...
                    st.warning(fWOC {woc_number} ไม่พบในฐานข้อมูล)
        ...
        def safe_int(val)
            try
                ...
                st.error(fเกิดข้อผิดพลาดในการแปลงค่าเป็นจำนวนเต็ม {e})
                return 0
        ...
        def safe_float(val)
            try
                ...
                st.error(fเกิดข้อผิดพลาดในการแปลงค่าเป็นจำนวนทศนิยม {e})
                return 0.0
        ...
        for _, row in df.iterrows()
            ...
            try
                ...
                st.error(fWOC {row['woc_number']} มีจำนวนชิ้นงานไม่ถูกต้อง {row['pieces_count']})
                continue
...
def receive_mode(dept_to)
    ...
    if diff_pct  2
        send_telegram_message(
            ...
            fจำนวนเดิม {job['pieces_count']}  จำนวนที่รับจริง {pieces_new}  คลาดเคลื่อน {diff_pct.2f}%
        )
    ...
    if st.button(รับเข้าและส่งต่อ)
        ...
        st.success(fรับ WOC {woc_selected} เรียบร้อยและเปลี่ยนสถานะเป็น {dept_to} Received)
...
def work_mode(dept)
    ...
    if st.button(เริ่มทำงาน)
        ...
        if job['status'] == TP Received or job['status'] == FI Received
            update_status(woc_selected, Completed)
            st.info(fสถานะของ WOC {woc_selected} ถูกเปลี่ยนเป็น Completed ก่อนเริ่มทำงาน)

        # เปลี่ยนสถานะของ WOC เป็น Working
        update_status(woc_selected, f{dept} Working)
        st.success(fเริ่มทำงาน WOC {woc_selected} ที่เครื่อง {machine_name})
...
def completion_mode()
    ...
    if st.button(บันทึก Completion)
        ...
        if diff_pct  2
            st.error(fจำนวนไม่ตรงกับจำนวนที่รับเข้า (คลาดเคลื่อน {diff_pct.2f}%))
            return

        update_status(woc_selected, Completed)
        st.success(fบันทึก Completion เรียบร้อย สถานะ WOC {woc_selected} เป็น Completed)
...
def report_mode()
    ...
    for d in depts
        ...
        wip_df = df[df[status].str.contains(fWIP-{d})]
        if wip_df.empty
            st.write(fแผนก {d} ไม่มีงาน WIP)
        else
            ...
            st.write(fแผนก {d})
            st.write(fจำนวนชิ้นงานรวมทั้งหมด {total_pieces,} ชิ้น)
...
def dashboard_mode()
    ...
    for wip_name, statuses in wip_map.items()
        ...
        st.markdown(fมีจำนวน {int(total),} ชิ้น)
    ...
    if not df_on_machine.empty
        st.write(fจำนวนงานที่กำลังทำงานบนเครื่องจักร {len(df_on_machine)} ชิ้น)
