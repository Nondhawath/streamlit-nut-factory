...
# === Update Status to On Machine ===
def update_status_to_on_machine(woc_number):
    with get_connection() as conn:
        cur = conn.cursor()
        cur.execute(
            "UPDATE job_tracking SET status = %s, on_machine_time = %s WHERE woc_number = %s",
...
def upload_wip_from_excel():
    ...
    if uploaded_file is not None:
        ...
        st.info("สามารถไปใช้งานต่อได้ในโหมด , Work, Completion, Dashboard, Report")
        report_mode()
...
def _mode(dept_to):
    ...
    # ปรับสถานะให้เป็น WIP และรอส่งต่อ
    ...
    if st.button("รับเข้าและส่งต่อ"):
        if not operator_name.strip():
            st.error("กรุณากรอกชื่อผู้ใช้งาน (Operator)")
            return

        # เปลี่ยนสถานะ WOC ที่ได้รับมา
        insert_job({
        ...
        # อัปเดตสถานะงานที่รับให้เป็น "Completed" หากสถานะเป็น "d"
        update_status(woc_selected, "Completed")

        st.success(f"รับ WOC {woc_selected} เรียบร้อยและเปลี่ยนสถานะเป็น {dept_to} d")
...
def work_mode(dept):
    ...
    if st.button("เริ่มทำงาน"):
        ...
        # ถ้าสถานะเป็น TP d หรือ FI d เปลี่ยนสถานะเป็น Completed ก่อน
        if job['status'] == "TP d" or job['status'] == "FI d":
            update_status(woc_selected, "Completed")
...
def dashboard_mode():
    ...
    wip_map = {
        "WIP-FM": ["FM Transfer TP", "FM Transfer OS", "FM Transfer FI"],
        "WIP-TP": ["TP d", "TP Transfer FI", "TP Working", "WIP-Tapping Work", "TP Transfer OS"],
        "WIP-OS": ["OS d", "OS Transfer FI"],
        "WIP-FI": ["FI d", "FI Working", "WIP-Final Work"],
        "Completed": ["Completed"]
    }
