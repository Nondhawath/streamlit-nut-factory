...
# ดึงชื่องานจากคอลัมน์แรก (ชื่อ รหัสงาน) ของชีท OS_part_code_master (ข้าม header แถวแรก)
job_codes = part_code_sheet.col_values(1)[1:]

# ดึงข้อมูล user (รหัส -> ชื่อ) จากชีท "ชื่อและรหัสพนักงาน"
employee_data = login_sheet.get_all_records()
# แปลงรหัสเป็น string เสมอ เพื่อให้ตรงกับ input ที่ได้จาก text_input (string)
...
if mode == "รับงานเข้า":
    ...
    # กรองแถวที่มีข้อมูลผู้ส่งซ่อม (ไม่ว่าง)
    repair_data = [r for r in records if r.get("ผู้ส่งซ่อม") and r.get("ผู้ส่งซ่อม") != ""]
    if repair_data:
